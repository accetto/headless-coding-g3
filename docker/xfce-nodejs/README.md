...
Therefore only the following image tags will be regularly built and published on the **Docker Hub** (with `Node.js LTS` by default):
...
- `vscode-firefox` adds [Visual Studio Code][vscode] and [Firefox][firefox] with the **plus features** (described in the sibling project README)
...
The container's directory `~/projects/samples` already contains the following simple testing applications:
